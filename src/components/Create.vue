  ...
  <div id="adicionar-livro">
      ...
      <form v-on:submit.prevent="addLivro">
          ...
          <div class="form-group">
              ...
              <input type="number" class="form-control" v-model="livro.Preco" id="preco" required>
          </div>
...
export default {
  ...
  methods: {
      addLivro: function () {
          ...
          if (isNaN(preco)) {
              alert('Preco deve ser um número')
              return false
          } else {
              this.livro.preco = this.livro.preco
          }
          this.$http
              .post('http://localhost:5000/create', this.livro, {
                  headers: {
              ...
              .then(
                  (response) => {
                      ...
                      this.$router.push('list')  //Faz retornar para a página de lista de livros
                  },
                  (response) => {
                      alert(response.body['mensagem'])
                  }
